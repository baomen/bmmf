...
namespace BaoMen.MultiMerchant.Merchant.DataAccess
{
    ...
    public partial class DownloadFile : DapperDataAccess<int,Entity.DownloadFile, Entity.DownloadFileFilter>
    {
        ...
        /// 数据库表名
        /// </summary>
        protected override string TableName { get { return "mch_download_file"; } }
        ...
        {
            string sql = $"INSERT INTO {TableName} (MerchantId,Type,OriginalFileName,FileName,ExtentionName,RelativePath,CreateTime,RelatedId) VALUES (@MerchantId,@Type,@OriginalFileName,@FileName,@ExtentionName,@RelativePath,@CreateTime,@RelatedId)";
        ...
        {
            string sql = $"UPDATE {TableName} SET MerchantId=@MerchantId,Type=@Type,OriginalFileName=@OriginalFileName,FileName=@FileName,ExtentionName=@ExtentionName,RelativePath=@RelativePath,RelatedId=@RelatedId WHERE Id=@Id";
            return new DapperCommand()
            {
                CommandText = sql,
                Parameters = item
        ...
        {
            string sql = $"SELECT {TableName}.Id,{TableName}.MerchantId,{TableName}.Type,{TableName}.OriginalFileName,{TableName}.FileName,{TableName}.ExtentionName,{TableName}.RelativePath,{TableName}.CreateTime,{TableName}.RelatedId FROM {TableName}";
            sql += " WHERE Id=@Id";
            return new DapperCommand()
            {
                CommandText = sql,
